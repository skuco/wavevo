...
export async function POST(request: Request) {
  ...
  try {
    ...
    const session: StudioRenderSession = { tracks, settings, masterVolume: settings.masterVolume, ...settings.view };
    await writeFile(path.join(directory, "studio.json"), JSON.stringify(session));
    await writeFile(path.join(directory, "metadata.json"), JSON.stringify({ name: "Wavevo studio", duration }));
    await createStudioVideo(id, mixPath, path.join(directory, `export.${settings.format}`), duration, settings, settings.format);
