...
import { announceRender, type useRenderJobs } from "./use-render-jobs";
import { formatTime } from "@/lib/format";
import { isActiveJob, type RenderJob } from "@/lib/render-job";
import { renderTimeLabel } from "@/lib/render-time";
...
export function RenderStatus({ center, onOpen }: { center: Center; onOpen: () => void }) {
  ...
  return <section className={`render-strip ${job?.status === "failed" ? "render-strip-failed" : ""}`} aria-label="Video render status">
    ...
    <div className="render-strip-copy"><strong>{center.error || (job?.status === "completed" ? "Your video is ready" : job?.status === "failed" ? "Video render failed" : job?.stage)}</strong><span title={job?.sessionName}>{job?.sessionName}{center.active.length > 1 && ` · ${center.active.length - 1} more in queue`}</span>{job && <span className="render-timing">{renderTimeLabel(job)}</span>}</div>
    ...
    <span className="visually-hidden" role="status">{center.notice?.status === "completed" ? `${center.notice.sessionName}: video ready to download. ${renderTimeLabel(center.notice)}.` : center.notice?.status === "failed" ? `${center.notice.sessionName}: render failed.` : ""}</span>
  </section>;
}
...
function RenderCard({ job }: { job: RenderJob }) {
  ...
  return <article className={`render-card render-card-${job.status}`}>
    ...
    <p className="render-timing">{renderTimeLabel(job)}</p>
